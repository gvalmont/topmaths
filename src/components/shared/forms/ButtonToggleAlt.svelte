...
<div {id} class="flex flex-col justify-start items-start">
  <div class="flex flex-row items-center space-x-1">
    ...
    />
    <label
      class="inline-block pl-[0.15rem] hover:cursor-pointer text-coopmaths-corpus dark:text-coopmathsdark-corpus text-opacity-70 text-sm {value
        ? 'font-semibold'
        : 'font-light'}"
      for="input-{id}"
    >
  ...
  <div class="flex flex-row p-2 font-light text-xs text-coopmaths-corpus dark:text-coopmathsdark-corpus text-opacity-70">
    {#if value}
      {explanations[0]}
    {:else}
      {explanations[1]}
