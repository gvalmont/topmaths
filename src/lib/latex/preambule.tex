...
\usepackage{ProfCollege} % EE : Pour pouvoir utiliser les arbres
...
\newcommand\theme[4]{%
  \fancyhead[C]{%
    ...
    \begin{tikzpicture}[remember picture,overlay]
      ...
      \node[anchor=west, fill=white, inner sep = 0pt] at ($(current page.north west)+(0.2,-2.3cm)$) {\footnotesize MathALEA};
    \end{tikzpicture}
    \begin{tikzpicture}[remember picture,overlay]
      \node[anchor=north east,inner sep=0pt] at ($(current page.north east)+(0,-.8cm)$) {};
      \node[anchor=east, fill=white] at ($(current page.north east)+(-2,-1.5cm)$) {\Huge \textcolor{couleur_theme}{\#} #2 \textcolor{couleur_theme}{\MakeUppercase{#3}}};
    \end{tikzpicture}
  }%
  \fancyfoot[R]{%
    \begin{tikzpicture}[remember picture,overlay]
