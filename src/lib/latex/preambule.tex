...
\NewDocumentCommand\Theme{o m m m m}{%
  ...
  \ifboolKV[Theme]{CANs}{%
    ...
    \ifboolKV[Theme]{Classiques}{%
      ...
      \colorlet{couleur_theme}{black}%
      \colorlet{couleur_numerotation}{couleur_theme}%
      \fancyhead[C]{\makebox[\textwidth]{\bfseries NOM :\hspace*{4cm} Prénom :\hspace*{4cm} #3}}
      \fancyhead[L]{#4}
      \fancyfoot[R]{#5}
    }{%
...
\newcommand\numb[1]{ % Dessin autour du numéro d'exercice
  \begin{tikzpicture}[overlay,scale=.8]%,yshift=-.3cm
    ...
    \draw (0.5,0) node[anchor=center,couleur_numerotation]{\large#1};
    \draw (-.4,.8) node[white,anchor=north west]{EX}; 
  \end{tikzpicture}
}
...
\newcommand\theme[4]{%
  \fancyhead[C]{%
    ...
    \begin{tikzpicture}[remember picture,overlay]
      ...
      \node[anchor=east, fill=white] at ($(current page.north east)+(-18.8,-2.3cm)$) {\footnotesize MathALEA};
    \end{tikzpicture}
    \begin{tikzpicture}[line cap=round,line join=round,remember picture, overlay, shift={(current page.north west)},yshift=-8.5cm]
      ...
      \node[color=white] at (10.5,5.5) {\LARGE \MakeUppercase{#4}};
    \end{tikzpicture}
    \begin{tikzpicture}[remember picture,overlay]
      \node[anchor=north east,inner sep=0pt] at ($(current page.north east)+(0,-.8cm)$) {};
      \node[anchor=east, fill=white] at ($(current page.north east)+(-2,-1.5cm)$) {\Huge #2 \textcolor{couleur_theme}{\MakeUppercase{#3}}};
    \end{tikzpicture}
  }%
  \fancyfoot[R]{%
    \begin{tikzpicture}[remember picture,overlay]
...
\newcommand\version[1]{
  \fancyhead[R]{
    \begin{tikzpicture}[remember picture,overlay]
    \node[anchor=north east,inner sep=0pt] at ($(current page.north east)+(-.5,-.5cm)$) {\large \textcolor{couleur_theme}{V#1}};
    \end{tikzpicture}
  }
}
