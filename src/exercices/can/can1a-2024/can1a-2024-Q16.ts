...
export default class PointsCourbe extends Exercice {
  ...
  nouvelleVersion () {
    ...
    if (this.canOfficielle) {
      ...
      this.reponse = texNombre(AinterB, 2) === texNombre(A * B, 2) ? 'VRAI' : 'FAUX'
    }
    ...
    \\end{aligned}$
    `
    this.correction += texNombre(AinterB, 2) === texNombre(A * B, 2)
    ...
    Les événements $A$ et $B$ sont donc indépendants.<br> L'affirmation est ${texteEnCouleurEtGras('VRAIE')} `
      : `<br> $P(A\\cap B)\\neq P(A)\\times P(B)$.<br>
    Les événements $A$ et $B$ ne sont donc pas indépendants. <br>L'affirmation est ${texteEnCouleurEtGras('FAUSSE')} `
    ...
    this.question = `$P(A\\cap B)=${texNombre(AinterB, 2)}$<br>
    $P(A)=${texNombre(A, 2)}$${sp(2)} ;${sp(2)}$P(B)=${texNombre(B, 2)}$<br>
    $A$ et $B$ sont indépendants.<br>` + qcm.texte
