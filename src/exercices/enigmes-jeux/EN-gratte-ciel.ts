...
export default class gratteciel extends Exercice {
  ...
  constructor() {
    ...
    const immeubles = Array.from(Array(this.sup).keys()).map(x => (x+1) * 10);

    this.consigne = 'Cette grille représente une ville vue du ciel.<br>'
    this.consigne += 'Chaque case contient un immeuble de '
    this.consigne += immeubles.slice(0,-1).join(', ')+" ou "+immeubles[this.sup-1] + ' étages.<br>'
    this.consigne += 'Les immeubles d’une même rangée, ligne ou colonne, sont tous de tailles différentes.<br>'
    this.consigne += 'Les informations données sur les bords indiquent le nombre d’immeubles visibles '
    this.consigne += 'sur la rangée correspondante par un observateur situé à cet endroit.<br>'
    this.consigne += 'Le but du jeu est de trouver la disposition des immeubles dans la grille.<br>'
  ...
  nouvelleVersion(): void {
    ...
    for (
      ...
      const tabColHeaders = corner.concat(north.map(x => x.toString())).concat(corner);
      const tabLineHeaders = west;
      const tabColFooters = corner.concat(south.map(x => x.toString())).concat(corner);
