...
export default class EgaliteFGLycee7 extends Exercice {
  ...
  nouvelleVersion() {
    ...
    let texte0 =
      'Calculer le salaire mensuel moyen des femmes dans cette entreprise.'
    if (this.interactif)
      texte0 +=
    ...
    let texte1 =
      'Calculer le manque à gagner annuel moyen pour une femme par rapport à un homme.'
    if (this.interactif)
      texte1 +=
    ...
    if (this.interactif)
      texte2 +=
        ajouteChampTexteMathLive(this, 2, '', { texteApres: 'jours' }) + '<br>'
    handleAnswers(this, 2, { reponse: { value: 52 } })
    const correction2 = `$14{,}2\\,\\%$ de $365$ jours $\\approx 51{,}8$, soit environ $${miseEnEvidence('52')}$ jours : les femmes travaillent donc symboliquement « gratuitement » les $52$ derniers jours de l'année, soit à partir d'environ le $9$ novembre (date proche de la « journée de l'égalité salariale » réellement observée chaque année en France).`
