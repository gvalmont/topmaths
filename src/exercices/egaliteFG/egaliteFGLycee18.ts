...
`) +
    ...
    this.commentaireApprofondir =
      ...
      ajouterLien(
        ...
        'Pour mieux connaître Maria Gaetana Agnesi',
      )
  ...
  nouvelleVersion() {
    ...
    const texte0 =
      texteGras('Partie A : expression de $x$ en fonction de $y$') +
      "<br>Le point $A$ appartient au cercle $\\mathcal{C}$ de diamètre $[OM]$. Démontrer que les coordonnées de $A$ vérifient l'égalité $u^2+y^2-2y=0$."
    const correction0 = `Le triangle $OAM$ est inscrit dans le cercle $\\mathcal{C}$ de diamètre $[OM]$ : d'après la propriété de l'angle inscrit dans un demi-cercle, il est rectangle en $A$, donc $\\vec{AO}\\cdot\\vec{AM}=0$. Avec $O(0\\,;\\,0)$, $M(0\\,;\\,2)$ et $A(u\\,;\\,y)$, on a $\\vec{AO}(-u\\,;\\,-y)$ et $\\vec{AM}(-u\\,;\\,2-y)$, d'où $(-u)\\times(-u)+(-y)\\times(2-y)=0$, soit $u^2-2y+y^2=0$, c'est-à-dire $${miseEnEvidence('u^2+y^2-2y=0')}$.`

    const texte1 =
      "Démontrer que l'égalité suivante est vraie : $\\dfrac{QA}{QO}=\\dfrac{MN}{MO}$."
    const correction1 =
    ...
    const correction2 = `$Q(0\\,;\\,y)$ et $A(u\\,;\\,y)$ ont la même ordonnée, donc $QA=u$ ; $Q(0\\,;\\,y)$ et $O(0\\,;\\,0)$ ont la même abscisse, donc $QO=y$ ; $M(0\\,;\\,2)$ et $N(x\\,;\\,2)$ ont la même ordonnée, donc $MN=x$ ; et $MO=2$. L'égalité $\\dfrac{QA}{QO}=\\dfrac{MN}{MO}$ devient donc $\\dfrac{u}{y}=\\dfrac{x}{2}$, soit $${miseEnEvidence('u=\\dfrac{xy}{2}')}$.`

    const texte3 =
      'Déduire des questions précédentes que : $A\\in\\mathcal{C}\\iff y=\\dfrac{8}{x^2+4}$.'
    const correction3 = `En remplaçant $u=\\dfrac{xy}{2}$ dans $u^2+y^2-2y=0$, on obtient $\\dfrac{x^2y^2}{4}+y^2-2y=0$, soit $y\\left(\\dfrac{x^2y}{4}+y-2\\right)=0$. Comme $y\\neq 0$ (sinon $A=O$), on a $\\dfrac{x^2y}{4}+y-2=0$, soit $y\\left(\\dfrac{x^2}{4}+1\\right)=2$, soit $y\\left(\\dfrac{x^2+4}{4}\\right)=2$, d'où $${miseEnEvidence('y=\\dfrac{8}{x^2+4}')}$. La sorcière d'Agnesi est donc la courbe représentative de la fonction $A$ définie sur $\\mathbb{R}$ par $A(x)=\\dfrac{8}{x^2+4}$.`
    ...
    const texteQ6 = 'La courbe $C_A$ possède-t-elle une asymptote horizontale ?'
    ...
    const correction6 = `Quand $x\\to\\pm\\infty$, $x^2+4\\to +\\infty$ donc $A(x)=\\dfrac{8}{x^2+4}\\to 0$ : la courbe admet la droite d'équation $${miseEnEvidence('y=0')}$ (l'axe des abscisses) comme asymptote horizontale en $+\\infty$ et en $-\\infty$.`
    ...
    const correction8 = `On calcule $A''(x)=\\dfrac{48x^2-64}{(x^2+4)^3}$, qui s'annule et change de signe pour $x^2=\\dfrac{4}{3}$, soit $x=\\pm\\dfrac{2}{\\sqrt{3}}=\\pm\\dfrac{2\\sqrt{3}}{3}\\approx \\pm ${miseEnEvidence('1{,}155')}$. La fonction $A$ est concave sur $\\left]-\\dfrac{2\\sqrt3}{3}\\,;\\,\\dfrac{2\\sqrt3}{3}\\right[$ et convexe à l'extérieur de cet intervalle.`

    let texte9 =
      "Calculer l'ordonnée de ces points d'inflexion, c'est-à-dire $A\\left(\\dfrac{2}{\\sqrt 3}\\right)$."
    if (this.interactif) texte9 += ajouteChampTexteMathLive(this, 9) + '<br>'
    handleAnswers(this, 9, { reponse: { value: 1.5 } })
    const correction9 = `$A\\left(\\dfrac{2}{\\sqrt3}\\right)=\\dfrac{8}{\\frac{4}{3}+4}=\\dfrac{8}{\\frac{16}{3}}=\\dfrac{24}{16}=${miseEnEvidence('1{,}5')}$.`
