...
export default class EgaliteFG8 extends ExerciceVraiFaux {
  ...
  constructor() {
    ...
    this.consigne +=
      ...
      (context.isHtml ? '<div class="not-prose" style="text-align:center; margin: 0.75rem 0;"><img src="/alea/images/egalite/mur-equerrage.jpg" alt="Contrôle d\'équerrage d\'un mur avec les points H, I, S" style="max-width:200px; width:100%; height:auto;"></div>' : '') +
    ...
    this.commentaireDebat =
      texteGras('Pour débattre') + '.<br>Pensez-vous que le métier de maçon est réservé uniquement aux hommes ?<br>Plus généralement, que pensez-vous de la place des femmes dans les métiers du bâtiment ?'
    this.besoinFormulaireCaseACocher = ['Afficher « Pour débattre »', true]
    this.sup = true
    this.commentaireApprofondir =
      texteGras('Pour aller plus loin') + '.<br>Pour en savoir plus sur le métier de maçon et découvrir qu\'il est accessible à toutes et à tous, vous pouvez consulter ' +
      ajouterLien('https://www.onisep.fr/ressources/univers-metier/metiers/macon-maconne', 'cette fiche métier complète proposée par l\'Onisep') +
      '.'
