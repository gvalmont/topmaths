...
export default function VariationDUneSuiteDefinieParRecurrence (this: any) {
  Exercice.call(this)
  this.titre = titre
    ...
    for (let i = 0, texte:string, texteCorr:string, cpt = 0, alea1:number, alea2:number ; i < this.nbQuestions && cpt < 50;) {
      switch (listeTypeDeQuestions[i]) { // listeTypeDeQuestions[i]
        case 1: // Suite arithmétique (u_{n+1}=u_n+a avec u_0=premier_terme)
          ...
          const premier_terme = randint(1, 39) * choice([-1, 1])
          alea1 = a
          alea2 = premier_terme
          
          texte = `Soit $(u_n)$ une suite définie pour tout entier $n\\in\\mathbb{N}$ par $u_0=${(premier_terme)}$ et $u_{n+1} = u_n ${ecritureAlgebrique(a)}$. `
          texte += '<br><br>'
        ...
        case 2: // Suite définie par u_{n+1}=u_n+b*n avec u_0=u_0
          ...
          const u_0 = randint(1, 39) * choice([-1, 1])
          alea1 = b
          alea2 = u_0
          
          texte = `Soit $(u_n)$ une suite définie pour tout entier $n\\in\\mathbb{N}$ par $u_0=${(u_0)}$ et $u_{n+1} = u_n ${ecritureAlgebrique(b)}n$.`
          texte += '<br><br>'
          texte += `Etudier le sens de variation de la suite $(u_n)$`
