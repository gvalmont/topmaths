...
export default class ComparerDesNombresAUn extends Exercice {
  constructor() {
    ...
    this.consigne =
      "Pour chacun des nombres suivants, indiquez s'il est strictement inférieur à $1$"
  }
  nouvelleVersion() {
    const fonctionsGeneratrices = shuffle(generateurs)
    let templates = ``
    let indexChamp = 1
    ...
    for (const generateur of fonctionsGeneratrices) {
      ...
      if ('fraction' in nombre) {
        templates += `${lettreMinusculeDepuisChiffre(indexChamp)}. $${nombre.fraction.texFraction}$ %{champ${indexChamp}}${sp(10)}`
        texteCorr += `${lettreMinusculeDepuisChiffre(indexChamp)}. $${nombre.fraction.texFraction} ${nombre.fraction.num < nombre.fraction.den ? miseEnEvidence('<') : nombre.fraction.num > nombre.fraction.den ? miseEnEvidence('>') : miseEnEvidence('=')} 1$ car le numérateur est ${nombre.fraction.num < nombre.fraction.den ? 'inférieur' : nombre.fraction.num > nombre.fraction.den ? 'supérieur' : 'égal'} au dénominateur.<br>`
        ...
        templates += `${lettreMinusculeDepuisChiffre(indexChamp)}. $\\dfrac{${texNombre(nombre.quotient.num, 3)}}{${texNombre(nombre.quotient.den, 3)}}$ %{champ${indexChamp}}${sp(10)}`
        texteCorr += `${lettreMinusculeDepuisChiffre(indexChamp)}. $\\dfrac{${texNombre(nombre.quotient.num, 3)}}{${texNombre(nombre.quotient.den, 3)}} ${nombre.quotient.num < nombre.quotient.den ? miseEnEvidence('<') : nombre.quotient.num > nombre.quotient.den ? miseEnEvidence('>') : miseEnEvidence('=')} 1$ car le numérateur est ${nombre.quotient.num < nombre.quotient.den ? 'inférieur' : nombre.quotient.num > nombre.quotient.den ? 'supérieur' : 'égal'} au dénominateur.<br>`
        ...
        templates += `${lettreMinusculeDepuisChiffre(indexChamp)}. $${nombre.expression}$ %{champ${indexChamp}}${sp(10)}`
        texteCorr += `${lettreMinusculeDepuisChiffre(indexChamp)}. $${nombre.expression} ${nombre.value < 1 ? miseEnEvidence('<') : nombre.value > 1 ? miseEnEvidence('>') : miseEnEvidence('=')} 1$ car le calcul donne $${texNombre(nombre.value, 3)}$ comme résultat.<br>`
        if (nombre.value < 1) {
          reponses[`champ${indexChamp}`] = { value: '<' }
