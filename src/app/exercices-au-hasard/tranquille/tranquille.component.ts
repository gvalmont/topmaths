...
@Component({
  selector: 'app-tranquille',
  templateUrl: './tranquille.component.html',
  styleUrls: ['./tranquille.component.css']
})
export class TranquilleComponent implements OnInit {
  infosModale: [string[], string, Date]

  constructor(public http: HttpClient, private viewportScroller: ViewportScroller) {
   ...
   */
  scrollBack() {
    this.viewportScroller.scrollToAnchor('descriptionTranquille')
  }
  ...
  lancerTranquille(niveauChoisi: string) {
    ...
    this.http.get<NiveauSequence[]>('assets/data/sequences.json').subscribe(niveaux => {
      ...
      this.http.get<NiveauObjectif[]>('assets/data/objectifs.json').subscribe(niveaux => {
        ...
        this.infosModale = [listeDesUrl, 'tranquille', new Date()]
      })
    })
  }
}
