...
#definition()[
  ...
  #grid(
    columns: (3fr, 1fr, 1fr, 4fr, 2fr, 3fr),
    align(center + horizon)[M appartient à (d)], image("6G7A-1.png"), [], align(center + horizon)[M n'appartient pas à (d)], image("6G7A-2.png"),
  )
]

#methode(titre: "Méthode pour construire le symétrique d'un point M par rapport à une droite (d)")[
  #let step(numero) = {
    align(center)[#image("6G7A-" + str(numero + 2) + ".png", height: 10em)#box(align(center + horizon, text(black, str(numero))), stroke: 1pt, radius: 100%, width: 16pt, height: 16pt)]
  }
  #grid(
    columns: 5,
