    ...
    files.map(async (file) => {
      ...
      if (stat.isDirectory()) {
        ...
        if (
          ...
          const matchRef = data.match(/export const refs = {[^]*'fr-ch': \[([^\]]*)\]/)
          if (matchRef) {
            const refsArray = matchRef[1].split(',').map(ref => ref.trim().replace(/'/g, ''))

            if (refsArray.length === 0) {
              console.error('\x1b[31m%s\x1b[0m', `Empty refs array in ${filePath}`)
            } else {
 ...
 */
const emptyRef2022 = readFileSync('tasks/emptyRefCH.json')
...
readInfos(exercicesDir, uuidMap)
  .then(() => {
    ...
    fs.writeFile('src/json/exercicesCH.json', JSON.stringify(exercices, null, 2))
    fs.writeFile('src/json/exercicesNonInteractifsCH.json', JSON.stringify(exercicesNonInteractifs.sort(), null, 2))
    uuidToUrl = handleExerciceSvelte(uuidToUrl)
    fs.writeFile('src/json/uuidsToUrlCH.json', JSON.stringify(uuidToUrl, null, 2))
    fs.writeFile('src/json/refToUuidCH.json', JSON.stringify(refToUuid, null, 2))
    ...
    fs.writeFile('src/json/referentielGeometrieDynamiqueCH.json', JSON.stringify(referentiel2022['Géométrie dynamique'], null, 2))
    delete referentiel2022['Géométrie dynamique']
    fs.writeFile('src/json/referentiel2022CH.json', JSON.stringify(referentiel2022, null, 2).replaceAll('"c3"', '"CM1/CM2"'))
  })
  .then(() => {
    console.log('uuidsToUrlCH, referentiel2022CH et referentielGeometrieDynamiqueCH ont été mis à jour')
  })
  .catch((err) => {
    console.error(err)
  })
